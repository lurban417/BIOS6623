...
model2 <- lm(tkvht_change ~ geom1 + geom2 + gabor1 + gabor2 + gabor3 + gabor4 + gabor5 + 
...
# Calculate RMSE (Root Mean Squared Error) for Model 1
rmse2 <- sqrt(mean((predictions1 - test_data$tkvht_change)^2))
cat("RMSE for Model 2: ", rmse1, "\n")
```
```{r}
# Model 3: Both baseline height-corrected total kidney volume and image features as predictors
model3 <- lm(tkvht_change ~ tkvht_base + geom1 + geom2 + gabor1 + gabor2 + gabor3 + gabor4 + gabor5 + 
              glcm1 + glcm2 + txti1 + txti2 + txti3 + txti4 + txti5 + 
              lbp1 + lbp2 + lbp3 + lbp4 + lbp5, 
              data = train_data_z)
